...
if (f>R)
    ...
    z=input('Enter the damping factor value '); %Damping Factor
    e=xd-x; %error
    ...
    e=abs(xd-x) %errorl
    I=[1 0 ;0 1];
while (e(1,1)>0.01 || e(2,1)>0.01) % while error is very less, the loop runs
    ...
    ad=((J')*inv((J*J')+(z^2*I)))*e; % Change in Joint angles 
    ...
    y_2=eval(y_2); % evaluating x coordinate
    x=[x_2;y_2];
